...
#        print(row)
        ...
        if 'cow' in c or 'dog' in c or 'person' in c:
            pygame.mixer.music.load('mixkit-censorship-beep-1082.wav')

            cx=int(x1+x2)//2
            cy=int(y1+y2)//2
            result_noice=cv2.pointPolygonTest(np.array(area1,np.int32),((cx,cy)),False)
            if result_noice>=0:
                # Play the audio file
